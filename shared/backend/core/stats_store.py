...
async def init_stats_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            await db.execute("ALTER TABLE device_heartbeats ADD COLUMN firmware_build_id TEXT")
        except aiosqlite.OperationalError:
            pass
        await db.commit()
...
async def _ensure_heartbeat_build_column(db) -> None:
    cursor = await db.execute("PRAGMA table_info(device_heartbeats)")
    columns = {str(row[1]) for row in await cursor.fetchall()}
    if "firmware_build_id" not in columns:
        await db.execute("ALTER TABLE device_heartbeats ADD COLUMN firmware_build_id TEXT")


async def log_heartbeat(mac: str, battery_voltage: float, wifi_rssi: Optional[int] = None,
                        firmware_build_id: Optional[str] = None):
    now = datetime.now().isoformat()
    db = await get_main_db()
    await _ensure_heartbeat_build_column(db)
    await db.execute(
        """INSERT INTO device_heartbeats
           (mac, battery_voltage, wifi_rssi, firmware_build_id, created_at)
           VALUES (?, ?, ?, ?, ?)""",
        (mac, battery_voltage, wifi_rssi, firmware_build_id, now),
    )
...
async def get_latest_heartbeat(mac: str) -> Optional[dict]:
    ...
    await _ensure_heartbeat_build_column(db)
    cursor = await db.execute(
        """SELECT battery_voltage, wifi_rssi, firmware_build_id, created_at FROM device_heartbeats
           WHERE mac = ?
           ORDER BY created_at DESC LIMIT 1""",
        (mac,),
    )
