...
class ConfigRequest(BaseModel):
    ...
    servers: list[dict] = Field(
        default_factory=list,
        max_length=5,
        description="后端服务器列表 [{url, role}]，role: primary|fallback|public；设备按顺序自动切换",
    )
...
class RenderQuery(BaseModel):
    ...
    fmt: str = Field(default="bmp", description="响应格式: bmp (默认) | structured (JSON v1)")
    delta: int = Field(default=0, ge=0, le=1, description="1=请求增量交付：模块无变化返回 noop，只下发变化的模块")
...
class DeviceHeartbeatRequest(BaseModel):
    ...
    firmware_build_id: Optional[str] = Field(default=None, max_length=80, pattern=r"^[a-zA-Z0-9._-]+$")
