...
# Do not download the historical WenQuanYi PCF mirror: that repository has no
# authoritative license file. Rendering remains usable through the OFL vector
# fonts above; the loader already falls back to those when no bitmap is present.
...
def _install_bitmap_fonts(force: bool) -> tuple[int, int, int]:
    ...
    if not BITMAP_FONT_URLS:
        print("Bitmap fonts: skipped (source license not established); using OFL vector fonts.")
        return 0, 0, 0
