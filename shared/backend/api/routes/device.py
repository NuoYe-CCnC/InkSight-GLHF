...
from core import gold_catchup, news_due_request
...
@router.post("/device/{mac}/gold/refresh")
async def device_gold_refresh(
    mac: str,
    payload: dict,
    _: bool = Depends(require_device_token),
):
    """Authenticated wake refresh for devices using the local backend path."""
    validate_mac_param(mac)
    try:
        return gold_catchup.catchup(
            request_id=payload.get("request_id"), reason="device_wake_direct")
    except ValueError as exc:
        return JSONResponse({"error": str(exc)}, status_code=400)


@router.post("/device/{mac}/news/check")
async def device_news_check(
    mac: str,
    payload: dict,
    _: bool = Depends(require_device_token),
):
    """Authenticated, idempotent check for the publisher-selected due issue."""
    validate_mac_param(mac)
    try:
        return news_due_request.check(payload.get("request_id"), reason="device-direct")
    except ValueError as exc:
        return JSONResponse({"error": str(exc)}, status_code=400)
...
async def post_device_heartbeat(
    ...
    from core import firmware_tasks
    ...
    try:
        ...
        build_id = str(body.firmware_build_id or "")
        if not firmware_tasks.accepts_upgrade_heartbeat(mac, build_id):
            raise
        issued_token = await generate_device_token(mac)
    await log_heartbeat(
        mac, body.battery_voltage or 3.3, body.wifi_rssi,
        firmware_build_id=body.firmware_build_id,
    )
    response = {"ok": True}
    if issued_token:
