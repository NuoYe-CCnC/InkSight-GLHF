...
from core.local_console_security import require_loopback
...
def _console_index_path() -> Path:
    return _backend_root() / "static" / "manager" / "index.html"


def _analytics_index_path() -> Path:
...
async def legacy_analytics_page():
    return FileResponse(_analytics_index_path(), media_type="text/html")
