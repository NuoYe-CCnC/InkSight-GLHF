...
@router.get("/auth/local-root-bootstrap")
async def local_root_bootstrap_status(request: Request):
    require_loopback(request)
    needed = not await local_admin.root_exists()
    return {
        "ok": True,
        "needed": needed,
        "bootstrap_token": issue_bootstrap_token() if needed else None,
    }


@router.post("/auth/local-root-bootstrap")
async def local_root_bootstrap(body: dict, response: Response, request: Request):
    require_loopback(request)
    require_same_origin(request)
    token = str(body.get("bootstrap_token") or "")
    if not verify_bootstrap_token(token):
        return JSONResponse({"error": "初始化令牌无效或已过期，请刷新页面。"}, status_code=403)
    try:
        username = str(body.get("username") or "")
        password = str(body.get("password") or "")
        user_id = await local_admin.create_first_root(username, password)
    except local_admin.AdminSetupError as exc:
        status = 409 if exc.code in {"root_exists", "username_exists"} else 400
        return JSONResponse({"error": str(exc), "code": exc.code}, status_code=status)
    session = create_session_token(user_id, username.strip())
