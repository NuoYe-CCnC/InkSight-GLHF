...
#if defined(EPD_PANEL_42_6_SSD1677) || defined(EPD_PANEL_42_SSD1683_BW) || defined(EPD_PANEL_42_DKE_RY683) || defined(EPD_PANEL_42_GDEM042F52)

#if defined(EPD_PANEL_42_6_SSD1677)
// 4.26 HAT：直接用微雪官方 EPD_4in26（esp32-waveshare-epd）驱动（其内部自带 bit-bang，
// 实测官方 bit-bang 可驱动此面板；本文件仅保留旧分支给其它面板/局刷路径兜底）
#include "EPD_4in26.h"
#include <driver/gpio.h>
#endif

// ── SPI 字节写：全为软件 bit-bang（与官方 DEV_SPI_WriteByte 一致）──
...
#else
    while (digitalRead(PIN_EPD_BUSY) == HIGH) {  // 经典：忙=高，闲=低（官方 epd4in26 ReadBusy）
#endif
        delay(10);
        if (millis() - t0 > timeoutMs) {
            Serial.printf("EPD busy TIMEOUT! BUSY=%d\n", digitalRead(PIN_EPD_BUSY));
            return;
        }
    }
...
static void epdReset() {
    ...
    digitalWrite(PIN_EPD_RST, LOW);  delay(4);
    digitalWrite(PIN_EPD_RST, HIGH); delay(20);
...
// ── SSD1677 (4.26" 800x480) full window: 10-bit X in pixels, 10-bit Y ──
// Register sequences per Waveshare epd4in26 (Arduino/epd4in26/epd4in26.cpp).
static void epdSetSsd1677FullWindow() {
    epdSendCommand(0x44);  // Set RAM X address range (10-bit, pixels)
    epdSendData(0x00);
    epdSendData(0x00);
    epdSendData((W - 1) & 0xFF);
    epdSendData(((W - 1) >> 8) & 0x03);

    epdSendCommand(0x45);  // Set RAM Y address range (10-bit, 官方 epd4in26: Y 递减 H-1→0)
    epdSendData((H - 1) & 0xFF);
    epdSendData(((H - 1) >> 8) & 0x03);
    epdSendData(0x00);
    epdSendData(0x00);
...
void gpioInit() {
#if PIN_EPD_PWR >= 0
    gpio_hold_dis((gpio_num_t)PIN_EPD_PWR);  // 唤醒后先释放深度睡眠期间的 PWR hold
    pinMode(PIN_EPD_PWR, OUTPUT);
    digitalWrite(PIN_EPD_PWR, HIGH);  // 面板供电使能（9脚模块）
#endif
...
void epdInit() {
    ...
    epdSendCommand(0x04);
    epdWaitBusy();
#elif defined(EPD_PANEL_42_6_SSD1677)
    // 4.26 HAT：官方 EPD_4in26_Init()（实测唯一能驱动此面板的路径）
    // 必须先跑官方 GPIO 配置（DEV_Module_Init）：实测主固件自身 gpioInit 会让面板假刷新
    DEV_Module_Init();
    EPD_4in26_Init();
#else
    epdReset();
...
void epdInitFast() {
    ...
    epdWaitBusy();
#elif defined(EPD_PANEL_42_6_SSD1677)
    // 4.26 HAT：官方 EPD_4in26_Init_Fast()
    DEV_Module_Init();
    EPD_4in26_Init_Fast();
#else
    epdReset();
...
#elif defined(EPD_PANEL_42_6_SSD1677)
    // 4.26 HAT：官方 EPD_4in26_Init + Display（实测唯一能驱动此面板的路径）
    {
        unsigned long tD0 = millis();
        DEV_Module_Init();
        EPD_4in26_Init();
        unsigned long tD1 = millis();
        EPD_4in26_Display((UBYTE *)image);
        unsigned long tD2 = millis();
        Serial.printf("[EPD-D] init=%lums display=%lums total=%lums BUSY=%d fb=%p\n",
                      tD1 - tD0, tD2 - tD1, tD2 - tD0,
                      digitalRead(PIN_EPD_BUSY), (const void *)image);
    }
#else
    epdInit();
...
#if defined(EPD_PANEL_42_6_SSD1677)
    EPD_4in26_Init();
    EPD_4in26_Display((UBYTE *)image);   // 官方全刷
...
void epdDisplayFast(const uint8_t *image) {
#if defined(EPD_PANEL_42_6_SSD1677)
    // 4.26 HAT：官方 Init_Fast + Display_Fast
    {
        unsigned long tF0 = millis();
        DEV_Module_Init();
        EPD_4in26_Init_Fast();
        EPD_4in26_Display_Fast((UBYTE *)image);
        Serial.printf("[EPD-F] official fast refresh %lums BUSY=%d\n",
                      millis() - tF0, digitalRead(PIN_EPD_BUSY));
    }
    return;
#endif
#if defined(EPD_PANEL_42_GDEM042F52)
    ...
    epdWaitBusy();
#if defined(EPD_PANEL_42_6_SSD1677)
    Serial.printf("[EPD-F] fast refresh %lums BUSY=%d\n", millis() - tF0, digitalRead(PIN_EPD_BUSY));
#endif
#endif
}
...
void epdPartialDisplayWithOld(uint8_t *data, const uint8_t *oldData, int xStart, int yStart, int xEnd, int yEnd) {
#if defined(EPD_PANEL_42_6_SSD1677)
    int xS = xStart / 8;                       // byte-aligned X window (SSD1677: 10-bit pixels)
    int xE = (xEnd - 1) / 8;
    int width = xE - xS + 1;
    int count = width * (yEnd - yStart);
    int xPixStart = xS * 8;
    int xPixEnd = xPixStart + width * 8 - 1;

    epdReset();
    epdWaitBusy();

    epdSendCommand(0x18);  // use internal temperature sensor
    epdSendData(0x80);

    epdSendCommand(0x3C);  // Border Waveform Control (partial)
    epdSendData(0x80);

    epdSendCommand(0x44);  // Set RAM X address range (10-bit pixels)
    epdSendData(xPixStart & 0xFF);
    epdSendData((xPixStart >> 8) & 0x03);
    epdSendData(xPixEnd & 0xFF);
    epdSendData((xPixEnd >> 8) & 0x03);

    epdSendCommand(0x45);  // Set RAM Y address range (10-bit pixels)
    epdSendData(yStart & 0xFF);
    epdSendData((yStart >> 8) & 0x03);
    epdSendData((yEnd - 1) & 0xFF);
    epdSendData(((yEnd - 1) >> 8) & 0x03);
    ...
    epdSendCommand(0x24);  // Write new B/W RAM
    ...
    if (oldData) {
        ...
        epdSendData(yStart & 0xFF);
        epdSendData((yStart >> 8) & 0x03);
        epdSendCommand(0x26);  // Write old B/W RAM for stable partial inversion
        for (int i = 0; i < count; i++)
            epdSendData(oldData[i]);
    }

    epdSendCommand(0x22);  // Display Update Control 2: partial sequence
    epdSendData(0xFF);
    epdSendCommand(0x20);  // Activate Display Update Sequence
    epdWaitBusy();
#elif defined(EPD_PANEL_42_DKE_RY683) || defined(EPD_PANEL_42_GDEM042F52)
