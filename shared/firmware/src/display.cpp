...
const uint8_t* getGlyph(char c) {
    ...
    static const uint8_t g_percent[]={0x23,0x13,0x08,0x64,0x62};
    ...
    switch (c) {
        ...
        case 'Q': return g_Q; case 'R': return g_R; case 'S': return g_S;
        case 'T': return g_T; case 'U': return g_U; case 'V': return g_V; case 'W': return g_W;
...
// ── 空窗时钟模式（P3）：大号 HH:MM + 时段字模 + 角标 ────────
void showClockMode() {
    memset(imgBuf, 0xFF, IMG_BUF_LEN);
    extern int curHour, curMin;

    // 顶部时段标签（上午/下午等，复用 16x16 汉字字模）
    drawPeriodLabel(imgBuf, W, H, 0, 0, W, 24);

    char buf[8];
    snprintf(buf, sizeof(buf), "%02d:%02d", curHour, curMin);
    int scale = 12;
    int w = textWidth(strlen(buf), scale);
    drawText(buf, (W - w) / 2, H / 2 - 50, scale);

    const char *tag = "NO SERVER";
    int tagScale = 2;
    int tagW = textWidth(strlen(tag), tagScale);
    drawText(tag, (W - tagW) / 2, H - 64, tagScale);
}
...
// RTC_DATA_ATTR：刷新计数跨深度睡眠保留（否则每次唤醒清零→每次全刷→整屏闪烁）
RTC_DATA_ATTR static uint32_t refreshCount = 0;
